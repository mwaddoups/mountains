                ...
                <div key={user.id} className={"h-auto sm:h-10 " + (expanded ? "flex w-full mr-1 my-1 items-center" : "mr-1")}>
                    ...
                    {expanded && <>
                        <p className="ml-3 text-sm text-gray-500">
                            ...
                            {currentUser?.is_committee ? `ICE: ${user.in_case_emergency ? user.in_case_emergency : "None given!"}` : ""}
                        </p>
                        ...
                        {currentUser?.is_committee && <button title="Toggle driving" className="ml-2 rounded bg-gray-300 w-6 h-6 p-1" onClick={toggleDriving(user.id)}><CarFrontFill /></button>}
