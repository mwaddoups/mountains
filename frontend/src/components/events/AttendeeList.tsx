...
interface AttendeeListProps {
    ...
    togglePaid: (userId: number) => (() => void);
    toggleAttendance: (userId: number) => (() => void);
}

export default function AttendeeList({ attendees, expanded, toggleWaitingList, toggleAttendance, togglePaid }: AttendeeListProps) {
            ...
            {attendees.map(user => (
                <div key={user.id} className={"h-auto sm:h-10 " + (expanded ? "flex w-full mr-1 my-1 items-center" : "mr-1")}>
                    ...
                    {expanded && <>
                        ...
                        {isEditor && <button title="Toggle driving" className="ml-2 rounded bg-gray-300 w-6 h-6 p-1" onClick={togglePaid(user.id)}><CashCoin /></button>}
