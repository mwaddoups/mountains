...
export default function AttendeeList({ attendees, expanded, toggleWaitingList, toggleAttendance }: AttendeeListProps) {
    ...
    return (
        <div className={expanded ? "w-full" : "flex flex-wrap"}>
            {attendees.map(user => (
                <div key={user.id} className={"h-10 " + (expanded ? "flex w-full mr-1 my-1 items-center" : "mr-1")}>
                    <Link to={`/platform/members/${user.id}`}>
                        <div className="w-10"><ProfilePicture user={user} /></div>
                    </Link>
                    {expanded && <>
                        <p className="ml-3 text-sm text-gray-500">
                            <span className={user.is_paid ? "text-blue-500 font-semibold" : ""}>{getName(user)}</span> {user.mobile_number ? `(${user.mobile_number})` : ""} {currentUser?.is_committee ? `ICE: ${user.in_case_emergency ? user.in_case_emergency : "None given!"}` : ""}
                        </p>
